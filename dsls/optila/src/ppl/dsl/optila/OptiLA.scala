...
*/
  ...
  abstract class Matrix[A] {
    ...
    def mapRows[B](f: Vector[A] => Vector[B]): Vector[A] 
  ...
  def aggregate[A](x: RangeVector, y: RangeVector)(f: (Int,Int) => A): Matrix[A]
  def aggregateIf[A](x: RangeVector, y: RangeVector)(c: (Int,Int) => Boolean)(f: (Int,Int) => A): Matrix[A]
